...
public class RainPanel extends JPanel implements PropertyChangeListener {
	...
	public double getMyRainRate() {
		String rainReading = myRainRateNum.getText();
		double rainValue = (Integer.parseUnsignedInt(rainReading.substring(0, rainReading.length()-4))*10)
				+ Integer.parseUnsignedInt(rainReading.substring(rainReading.length()-3, rainReading.length()-2));
		return rainValue / 10;
	}
	/////////// }
